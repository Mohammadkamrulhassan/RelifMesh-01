...
### Need Calculation Module (v2)

| TC ID | Test Case | Input | Expected Output | Status |
|-------|-----------|-------|-----------------|--------|
| TC-NEED01 | Calculate ward need from demographics | Valid ward ID with HH census | `assessment.calculated_qty = sum(HH_members) × per_person_rate` | [ ] |
| TC-NEED02 | Override calculated need | assessmentId + overrideQty + reason | `assessment.override_qty` set, heatmap picks override | [ ] |
| TC-NEED03 | Heatmap endpoint returns aggregated data | areaId = district | Array of ward-level `{ areaId, lat, lng, intensity }` | [ ] |
| TC-NEED04 | Zero population ward returns zero need | Ward with 0 HH | `calculated_qty = 0` | [ ] |
| TC-NEED05 | Unauthorized user cannot override need | NGO Worker token + override request | 403 Forbidden | [ ] |

### Pledge Module (v2)

| TC ID | Test Case | Input | Expected Output | Status |
|-------|-----------|-------|-----------------|--------|
| TC-PLEDGE01 | Declare valid pledge | sourceType=DONOR, areaId, itemCatId, qty | 201 Created, status=PENDING | [ ] |
| TC-PLEDGE02 | List pledges filtered by jurisdiction | Upazila Officer token | Only pledges in their area returned | [ ] |
| TC-PLEDGE03 | Public sees pledge summary on heatmap | GET /v1/need/heatmap/{areaId} | `total_pledged_qty` included in response | [ ] |
| TC-PLEDGE04 | Cancel own pledge | Valid pledgeId + authenticated as source | Status = CANCELLED | [ ] |
| TC-PLEDGE05 | Link distribution to pledge | distribution with pledgeId FK | pledge.remaining_qty decreases by distro qty | [ ] |
...
- No NID or Personl name visible
- Map shows correct union markers
- Item totals correct

---

### UAT-05 — Need Assessment & Heatmap (v2)
**Actor:** Simulated Upazila Officer
**Scenario:** Run need calculation for all wards in a union, verify heatmap overlay, and override a value.
**Pass Criteria:**
- Calculated need matches `sum(HH_members) × per_person_rate` per ward
- Override value appears on heatmap instead of calculated
- Heatmap intensity gradient (green → yellow → red) visually matches need severity

---

### UAT-06 — Pledge-to-Fulfillment Flow (v2)
**Actor:** Simulated Outside Individual Donor, then UP Official
**Scenario:** Donor pledges rice for a specific union; UP Official logs a distribution referencing that pledge.
**Pass Criteria:**
- Pledge created with PENDING status (public-accessible for non-registered donors)
- Admin can view and filter pledges by area
- Distribution linked to pledge decreases `remaining_qty`
- Pledge auto-transitions to COMPLETED when `remaining_qty = 0`
...
|----------|-------------|--------|--------|---------|
| Unit Tests (Phase 1) | 29 | 29 | 0 | 0 |
| Integration Tests (Phase 1) | 7 | 7 | 0 | 0 |
| UAT Scenarios (Phase 1) | 4 | — | — | Manual verification pending |
| Phase 2 v2 Tests (Planned) | 10 | — | — | Not yet implemented |
| **Total** | **46** | **36** | **0** | — |
