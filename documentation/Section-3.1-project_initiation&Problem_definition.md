...
### 3.1.3.1 Case Study: Feni District Floods (2024 & 2025)
...
**Feni administrative makeup:**
- 6 upazilas: Feni Sadar, Sonagazi, Chhagalnaiya, Daganbhuiyan, Parshuram, Fulgazi
- 45 unions
- ~570 villages

Documented worst-hit unions (e.g., Kalidaha Union in Feni Sadar Upazila) serve as concrete examples throughout this report's UI mockups and heatmap demonstrations.

**Design implication:** Because mobile network infrastructure itself failed during the disaster, offline-first design (detailed in Section 3.6) is not optional — it is the defining constraint of this problem domain. The 2025 floods in the same region confirmed the recurring nature of this threat pattern.
...
| 6 | Heatmap loads with correct served/remaining split | Yes | Heatmap renders correctly for a test ward with known distribution data | Yes, using Leaflet.heat + aggregated endpoint | Transparency goal | By Module 10 |
| 7 | Need-calculation accuracy within Sphere-standard tolerance | Yes | Auto-calculated need per household matches Sphere reference rates (2,100 kcal/day baseline) within ±2% | Yes, using pre-seeded per-person ration rates | Enables needs-based coordination | By Module 10 |
...
**Note:** GIS/map feature ownership (heatmap, drill-down, pledge markers) is shared between Kamrul (backend data/api) and Sayeda/Nahid (frontend map UI), extending the existing Section 3.6/3.7 tech-stack ownership.
